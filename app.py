...
if not levels.empty:
    ...
    for level,(label,desc) in enumerate(MATURITY[lang]):
        ...
        maturity_cards.append(f'<article class="maturity-card {stage_class}" style="--level:{level_colors[level]};--soft:{softs[level]};--height:{heights[level]}px"><div class="circle">{MATURITY_ICONS[level]}</div><small>{LEVEL_LABEL[lang]} {level}</small><h3>{label}</h3><p>{desc}</p><div class="maturity-count"><b>{count}</b> {c["participants"]}<div class="share-track"><i style="width:{percentage:.1f}%"></i></div><strong>{percentage:.0f}%</strong></div></article>')
